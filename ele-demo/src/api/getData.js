...
export const cityGuess = () => new Promise((resolve, reject) => {
    fetch('https://elm.cangdu.org/v1/cities?type=guess').then(res => res.json()).then(data => resolve(data)).catch(err => reject(err));
});
export const hotcity = () => new Promise((resolve, reject) => {
    fetch('https://elm.cangdu.org/v1/cities?type=hot').then(res => res.json()).then(data => resolve(data)).catch(err => reject(err));
});
...
//     fetch('').then(res => res.json()).then((data) => resolve(data)).catch(err => reject(err));
// })
...
export const msiteFoodTypes = (geohash) => new Promise((resolve, reject) => {
    ...
    http.get(url, { geohash, group_type: '1', 'flags[]': 'F' }).then(res => resolve(res.data))
})